...
def create_combined_plot(run_name, model_name, last_iter=None, reinflate_list=None):
    ...
    if reinflate_list is not None:
        reinflate_iters = calculate_reinflation_iters(reinflate_list)
        print(f"\nUsing custom reinflation list: {reinflate_list}")
    else:
    ...
    category_colors = {
        'Hydraulic Conductivity': "#9247a5",
        'Storage': "#20a773",
        'Recharge': "#2b539c",
        'GHB Conductance': "#bd1616",
        'GHB Head': '#9b59b6',
        'GHB Other': '#95a5a6',
        'Other': '#34495e'
    }
    ...
    # Get subcategories for each main category with color gradients
    import matplotlib.colors as mcolors

    # Define color gradients for each category (start_color, end_color)
    category_gradients = {
        'Hydraulic Conductivity': ('#e74c3c', '#9b59b6'),  # red to purple
        'Storage': ('#3498db', '#27ae60'),                  # blue to green
        'Recharge': ('#1a5276', '#5dade2'),                  # dark blue to blue
        'GHB Conductance': ('#f39c12', '#922b21'),          # golden to dark red
    }
    ...
    for category in main_categories:
        ...
        if category in category_gradients:
            start_color, end_color = category_gradients[category]
            start_rgb = mcolors.to_rgb(start_color)
            end_rgb = mcolors.to_rgb(end_color)

            for j, subcat in enumerate(cat_subcats):
                # Interpolate between start and end colors
                t = j / max(1, n_subcats - 1) if n_subcats > 1 else 0.5
                color = tuple(start_rgb[i] + t * (end_rgb[i] - start_rgb[i]) for i in range(3))
                subcategory_colors[subcat] = color
        else:
            # Fallback to base color variations
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help='Custom reinflation iteration counts (e.g., --reinflate-iters 5 5 5 5 5 5 5 5 for 8 reinflations of 5 iters each)'
    )
