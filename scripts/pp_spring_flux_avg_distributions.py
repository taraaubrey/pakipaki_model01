"""
pp_spring_flux_avg_distributions.py - Compare prior vs posterior average spring flux distributions

Compares prior (iter 0) and posterior distributions for average spring flux predictions
(averaged across all spring cells) showing present vs future across different stress periods.

Usage:
    python scripts/pp_spring_flux_avg_distributions.py run_name [options]

Examples:
    python scripts/pp_spring_flux_avg_distributions.py local_run34 --post-iter 19
    python scripts/pp_spring_flux_avg_distributions.py run36 --run-name run36_62859_noGHBheads --post-iter 39
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Compare prior vs posterior average spring flux distributions'
    )
    parser.add_argument(
        'model_name',
        type=str,
...
def get_spring_obs_by_kper_kstp(obs_data):
    """
...
def create_spring_avg_distribution_plot(run_name, model_name, post_iter=19, filter_file=None,
    ...
    """
    Create distribution plot comparing prior vs posterior for average spring flux.
    """
    # Set suffix for output files
    file_suffix = suffix if suffix else ''
    print(f"\n{'='*80}")
    print(f"AVERAGE SPRING FLUX DISTRIBUTION ANALYSIS: {run_name}")
    print(f"{'='*80}")

    # Setup paths
    master_dir = os.path.join('models', run_name, 'pest', 'master_ies')
    output_dir = os.path.join('models', run_name, 'pp_predictions_springflux_avg')
    ...
    # Get spring observations (all cells)
    obs_dict, spring_df = get_spring_obs_by_kper_kstp(obs_data)

    # Count unique cells
    unique_cells = spring_df[['i', 'j']].drop_duplicates()
    n_cells = len(unique_cells)
    print(f"\nSpring flux observations: {len(spring_df)} total across {n_cells} cells")
    ...
    for comp in comparisons:
        ...
        print(f"\n{comp['name']}:")
        print(f"  Present {comp['present']}: {len(comp['present_obs'])} cells")
        print(f"  Past {comp['past']}: {len(comp['past_obs'])} cells")
    ...
    all_obs = []
    for comp in comparisons:
        all_obs.extend(comp['present_obs'])
        all_obs.extend(comp['past_obs'])
    ...
    # Calculate average flux for each realization and kper/kstp
    print("\nCalculating average flux across all cells...")

    def calc_avg_flux(oe, obs_names):
        """Calculate average flux across all cells for each realization."""
        valid_cols = [col for col in obs_names if col in oe.columns]
        if len(valid_cols) == 0:
            return np.array([])
        return oe[valid_cols].mean(axis=1).values
    ...
    # Build dataframe with average spring flux data
    spring_data_rows = []
    for comp in comparisons:
        ...
        if len(present_obs) > 0:
            prior_avg = calc_avg_flux(prior_oe, present_obs)
            post_avg = calc_avg_flux(post_oe, present_obs)

            for i, real_id in enumerate(prior_oe.index):
            ...
            for i, real_id in enumerate(post_oe.index):
                spring_data_rows.append({
                    'comparison': comp_name,
                    'period': 'present',
                    'realization': real_id,
        ...
        if len(past_obs) > 0:
            prior_avg = calc_avg_flux(prior_oe, past_obs)
            post_avg = calc_avg_flux(post_oe, past_obs)
    ...
    spring_data_file = os.path.join(data_dir, f'{run_name}_spring_flux_avg_data{file_suffix}.csv')
    ...
    log_file = os.path.join(data_dir, f'{run_name}_spring_flux_avg_log{file_suffix}.json')
    ...
    for col_idx, comp in enumerate(comparisons):
        ...
        if len(present_obs) == 0 or len(past_obs) == 0:
        ...
        # Get average data
        prior_present = calc_avg_flux(prior_oe, present_obs)
        prior_past = calc_avg_flux(prior_oe, past_obs)
        post_present = calc_avg_flux(post_oe, present_obs)
        post_past = calc_avg_flux(post_oe, past_obs)
        ...
        ax_present.set_xlabel('Avg Spring Flux (m³/d)', fontsize=6)
        ...
        ax_past.set_xlabel('Avg Spring Flux (m³/d)', fontsize=6)
        ...
        ax_diff.set_xlabel('Avg Flux Difference (m³/d)', fontsize=6)
    ...
    # Save figure
    output_path = os.path.join(output_dir, f'{run_name}_spring_flux_avg_predictions{file_suffix}.png')
    ...
    if len(kper2_obs) == 0 or len(kper4_obs) == 0:
        ...
        all_temporal_obs = []
        for obs_list in kper2_obs.values():
            all_temporal_obs.extend(obs_list)
        for obs_list in kper4_obs.values():
            all_temporal_obs.extend(obs_list)
        ...
        # Plot prior ensemble lines (grey) - average across cells
        for real_id in prior_temporal.index:
            vals = [prior_temporal.loc[real_id, kper2_obs[kstp]].mean() for kstp in kstps_2]
            ax_present.plot(kstps_2, vals, color=colors['prior'], alpha=0.15, linewidth=0.5)

        # Plot posterior ensemble lines (orange) - average across cells
        for real_id in post_temporal.index:
            vals = [post_temporal.loc[real_id, kper2_obs[kstp]].mean() for kstp in kstps_2]
            ax_present.plot(kstps_2, vals, color=colors['post_present'], alpha=0.4, linewidth=0.5)

        # Plot medians of average
        prior_p50 = [np.median([prior_temporal.loc[r, kper2_obs[kstp]].mean() for r in prior_temporal.index]) for kstp in kstps_2]
        post_p50 = [np.median([post_temporal.loc[r, kper2_obs[kstp]].mean() for r in post_temporal.index]) for kstp in kstps_2]
        ...
        all_post_vals_2 = np.array([post_temporal.loc[r, kper2_obs[kstp]].mean() for r in post_temporal.index for kstp in kstps_2])
        ax_present.set_ylim(np.min(all_post_vals_2), np.max(all_post_vals_2))

        ax_present.set_xlabel('Time step (kstp)', fontsize=6)
        ax_present.set_ylabel('Avg Spring Flux (m³/d)', fontsize=6)
        ...
        # Plot prior ensemble lines (grey) - average across cells
        for real_id in prior_temporal.index:
            vals = [prior_temporal.loc[real_id, kper4_obs[kstp]].mean() for kstp in kstps_4]
            ax_past.plot(kstps_4, vals, color=colors['prior'], alpha=0.15, linewidth=0.5)

        # Plot posterior ensemble lines (purple) - average across cells
        for real_id in post_temporal.index:
            vals = [post_temporal.loc[real_id, kper4_obs[kstp]].mean() for kstp in kstps_4]
            ax_past.plot(kstps_4, vals, color=colors['post_past'], alpha=0.15, linewidth=0.5)

        # Plot medians of average
        prior_p50_4 = [np.median([prior_temporal.loc[r, kper4_obs[kstp]].mean() for r in prior_temporal.index]) for kstp in kstps_4]
        post_p50_4 = [np.median([post_temporal.loc[r, kper4_obs[kstp]].mean() for r in post_temporal.index]) for kstp in kstps_4]
        ...
        # Set y-axis limits based on posterior values
        all_post_vals_4 = np.array([post_temporal.loc[r, kper4_obs[kstp]].mean() for r in post_temporal.index for kstp in kstps_4])
        ax_past.set_ylim(np.min(all_post_vals_4), np.max(all_post_vals_4))

        ax_past.set_xlabel('Time step (kstp)', fontsize=6)
        ax_past.set_ylabel('Avg Spring Flux (m³/d)', fontsize=6)
        ...
        # Save temporal plot
        temporal_path = os.path.join(output_dir, f'{run_name}_spring_flux_avg_temporal{file_suffix}.png')
    ...
    for kper, kstp in key_timesteps:
        obs_list = obs_dict.get((kper, kstp), [])
        key_obs_names.extend(obs_list)
    ...
    if len(key_obs_names) > 0:
        ...
        for kper, kstp in key_timesteps:
            obs_list = obs_dict.get((kper, kstp), [])
            valid_cols = [col for col in obs_list if col in post_key.columns]

            if len(valid_cols) > 0:
                # Calculate average across cells for each realization
                prior_avg = prior_key[valid_cols].mean(axis=1).values
                post_avg = post_key[valid_cols].mean(axis=1).values

                key_stats_rows.append({
                    'kper': kper,
                    'kstp': kstp,
                    'n_cells': len(valid_cols),
                    'prior_min': np.min(prior_avg),
                    'prior_max': np.max(prior_avg),
                    'prior_mean': np.mean(prior_avg),
                    'prior_median': np.median(prior_avg),
                    'post_min': np.min(post_avg),
                    'post_max': np.max(post_avg),
                    'post_mean': np.mean(post_avg),
                    'post_median': np.median(post_avg),
                })
            else:
                key_stats_rows.append({
                    'kper': kper,
        ...
        key_stats_file = os.path.join(output_dir, f'{run_name}_spring_flux_avg_key_stats{file_suffix}.csv')
...
if __name__ == '__main__':
    ...
    create_spring_avg_distribution_plot(
        run_name=run_name,
        model_name=model_name,
