...
def main():
    ...
    TEMP_DIR = r"C:\Users\tfo46\e_Python\e_projects\pakipaki_01\models\local_run4\pest\master_ies_copy"

    pst_name = f"local_run4.pst"
    pst = pyemu.Pst(os.path.join(TEMP_DIR, pst_name))

    # pest options
    pst.pestpp_options['ies_parameter_ensemble'] = 'local_run4.12.par.csv'
    pst.pestpp_options['ies_observation_ensemble'] = 'local_run4.obs+noise.csv'
    pst.pestpp_options["ies_restart_observation_ensemble"] = 'local_run4.12.obs.csv'
    pst.pestpp_options['overdue_giveup_fac'] = 10
    pst.pestpp_options['overdue_giveup_minutes'] = 15
    pst.pestpp_options["ies_n_iter_reinflate"] = 1, 4, 4
    pst.pestpp_options["ies_reinflate_factor"] = 1.1

    pst.control_data.noptmax = 9 # number of IES iterations (to accomodate reinflation)

    # write a new pst file
    new_pst = f"local_run4_restart.pst"
