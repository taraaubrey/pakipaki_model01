"""
pp_array_stats.py - Extract array observation statistics from posterior ensemble

Extracts array observations (arr-h, arr-spq, arr-awq, etc.) for a specific kper/kstp
and calculates statistics (mean, median, percentiles) across realizations for each cell.

Usage:
    python scripts/pp_array_stats.py run_name [options]

Examples:
    python scripts/pp_array_stats.py local_run34 --oname arr-h --kper 1 --kstp 1 --post-iter 19
    python scripts/pp_array_stats.py local_run34 --oname arr-spq --kper 1 --kstp 1 --post-iter 19
    python scripts/pp_array_stats.py local_run34 --oname arr-awq --kper 2 --kstp 52 --post-iter 19
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Extract array observation statistics from posterior ensemble'
    )
    parser.add_argument(
        'model_name',
        type=str,
    ...
    parser.add_argument('--oname', type=str, default='arr-awq',
                       help='Observation name type (default: arr-h). Options: arr-h, arr-spq, arr-awq, arr-confq')
    ...
    parser.add_argument('--filter-file', type=str, default=r"models\run36_62859_noGHBheads\filtered_realizations.csv",
...
def create_array_stats(run_name, model_name, oname='arr-h', post_iter=19, kper=1, kstp=1,
                       filter_file=None, suffix=None, vmin=None, vmax=None):
    """
    Extract array observation statistics from posterior ensemble.
    """
    ...
    print(f"\n{'='*80}")
    print(f"ARRAY STATISTICS: {run_name}")
    ...
    # Filter to specified oname observations
    arr_obs = obs_data[obs_data['oname'] == oname].copy()
    print(f"\nTotal {oname} observations: {len(arr_obs)}")

    if len(arr_obs) == 0:
        print(f"Error: No observations found with oname='{oname}'")
        print(f"Available onames: {obs_data['oname'].unique().tolist()}")
        return

    # Convert kper/kstp to numeric
    arr_obs['kper'] = pd.to_numeric(arr_obs['kper'], errors='coerce')
    arr_obs['kstp'] = pd.to_numeric(arr_obs['kstp'], errors='coerce')

    # Filter by kper/kstp
    arr_obs_filtered = arr_obs[(arr_obs['kper'] == kper) & (arr_obs['kstp'] == kstp)]
    print(f"{oname} observations for kper={kper}, kstp={kstp}: {len(arr_obs_filtered)}")

    if len(arr_obs_filtered) == 0:
        print("Error: No observations found for this kper/kstp")
        available_kpers = arr_obs['kper'].unique()
        print(f"Available kpers: {sorted(available_kpers)}")
        return

    # Get grid dimensions from i, j indices
    arr_obs_filtered['i'] = pd.to_numeric(arr_obs_filtered['i'], errors='coerce')
    arr_obs_filtered['j'] = pd.to_numeric(arr_obs_filtered['j'], errors='coerce')

    nrow = int(arr_obs_filtered['i'].max()) + 1
    ncol = int(arr_obs_filtered['j'].max()) + 1
    ...
    obs_names = arr_obs_filtered.index.tolist()
    ...
    post_oe.index = post_oe.index.astype(str)
    # Apply filter
    if filtered_ids:
    ...
    n_cells = 0
    for obs_name, obs_row in arr_obs_filtered.iterrows():
    ...
    for name, arr in stats_arrays.items():
        filename = f'{run_name}_{oname_clean}_kper{kper}_kstp{kstp}_{name}{file_suffix}.npy'
    ...
    for name, arr in stats_arrays.items():
        filename = f'{run_name}_{oname_clean}_kper{kper}_kstp{kstp}_{name}{file_suffix}.csv'
        filepath = os.path.join(arrays_dir, filename)
    ...
    for idx, (name, arr) in enumerate(stats_arrays.items()):
        ...
        if vmin is None:
            vmin = np.nanmin(masked_arr)
        if vmax is None:
            vmax = np.nanmax(masked_arr)

        im = ax.imshow(masked_arr, cmap='viridis', aspect='equal')
    ...
    plt.suptitle(f'{oname} Statistics - kper={kper}, kstp={kstp}', fontsize=12, fontweight='bold')
    plt.tight_layout()

    plot_path = os.path.join(output_dir, f'{run_name}_{oname_clean}_kper{kper}_kstp{kstp}_arrays{file_suffix}.png')
    ...
    for idx, (name, arr) in enumerate(stats_arrays.items()):
        ...
        if len(valid_vals) > 0:
            min = np.percentile(valid_vals, 5)
            max = np.percentile(valid_vals, 95)
            valid_vals = valid_vals[(valid_vals >= min) & (valid_vals <= max)]
    ...
    plt.suptitle(f'{oname} Statistics Histograms - kper={kper}, kstp={kstp}', fontsize=12, fontweight='bold')
    plt.tight_layout()

    hist_path = os.path.join(output_dir, f'{run_name}_{oname_clean}_kper{kper}_kstp{kstp}_histograms{file_suffix}.png')
    ...
    im = ax.imshow(uncertainty, cmap='YlOrRd', aspect='equal')
    ax.set_title(f'{oname} Uncertainty (P95 - P05) - kper={kper}, kstp={kstp}', fontsize=12, fontweight='bold')
    ax.set_xlabel('Column', fontsize=10)
    ax.set_ylabel('Row', fontsize=10)
    plt.colorbar(im, ax=ax, label='Uncertainty')

    unc_path = os.path.join(output_dir, f'{run_name}_{oname_clean}_kper{kper}_kstp{kstp}_uncertainty{file_suffix}.png')
    ...
    summary_path = os.path.join(output_dir, f'{run_name}_{oname_clean}_kper{kper}_kstp{kstp}_summary{file_suffix}.csv')
